...
namespace MAF.Assistants.Utility
{
    public static class Persistence
    {
        ...
        /// <summary>
        /// Saves the state of the specified <see cref="AgentSession"/> to a file in JSON format.
        /// </summary>
        /// <remarks>The method serializes the state of the provided <see cref="AgentSession"/> into JSON
        ...
        /// <param name="agent">The <see cref="AIAgent"/> instance used to serialize the session.</param>
        /// <param name="session">The <see cref="AgentSession"/> instance representing the conversation to save. Cannot be <see
        /// langword="null"/>.</param>
        ...
        public static async Task SaveConversationToFileAsync(AIAgent agent, AgentSession session, string fileName = "agent_thread.json")
        {
            // Serialize the session state
            JsonElement serialized = await agent.SerializeSessionAsync(session, JsonSerializerOptions.Web);
        ...
        /// <summary>
        /// Asynchronously loads a conversation from a specified JSON file and restores the session state for the given
        /// agent.
        /// </summary>
        /// <remarks>The method reads the serialized conversation from the specified file, deserializes
        /// it, and restores the session state for the provided agent. The file is expected to be located in the
        /// temporary file directory.</remarks>
        /// <param name="agent">The <see cref="AIAgent"/> instance for which the conversation session will be restored.</param>
        ...
        /// cref="AgentSession"/> instance representing the loaded conversation.</returns>
        /// <exception cref="FileNotFoundException">Thrown if the specified file does not exist at the expected location.</exception>
        public static async Task<AgentSession> LoadConversationFromFileAsync(AIAgent agent, string fileName = "agent_thread.json")
        {
            string filePath = Path.Combine(TempFileLocation, fileName);
            ...
            // Deserialize to restore the session state
            AgentSession session = await agent.DeserializeSessionAsync(reloaded, JsonSerializerOptions.Web);
